...
Future<List<Map<String, dynamic>>> getPdfList() async {
  ...
  return db.collection("quants").get().then(
    (querySnapshot) {
...
void downloadAndOpenPdf(BuildContext context, String url) async {
  ...
  try {
    ...
    Navigator.pop(context); 
    await OpenFile.open(filePath); // Open the PDF immediately after downloading
  } catch (error) {
    print('Error downloading PDF: $error');
  }
}

class quants extends StatefulWidget {
  const quants({super.key});

  @override
  State<quants> createState() => _quantsState();
}

class _quantsState extends State<quants> {
